    ...
    <div className="pt-5 relative">
      ...
      <div className="md:flex px-5 absolute w-full w-[calc(100%+2.5rem)] left-[-1.25rem] top-[60%] bg-gray-200">
          ...
          <div className="w-full mr-auto md:block flex gap-5">
            ...
            <div className="w-2/5 md:w-full flex-col flex z-30 gap-4">
              {session?.user.username == router.query.username ?
                <Link className="p-2 h-full bg-white mt-0 md:mt-3 rounded-lg text-center flex place-items-center place-content-center text-xl" href="/user/edit">
                  Edit profile
                </Link> :
                null
              }
              <Link href={`/user/${user.username}/friends`}
                    className="md:hidden h-full block p-2 px-5 md:mt-2 rounded-lg bg-white flex place-items-center place-content-center text-xl">
                Friends
              </Link>
            </div>
          </div>
        ...
        <main className="mt-4 md:mt-0 z-20 flex flex-col gap-4 md:w-1/2 mx-auto md:px-4">
          ...
          {user.description ?
            <div className="p-3 bg-white rounded-lg text-xl">
              {user.description}
            </div>
            : null
          }
